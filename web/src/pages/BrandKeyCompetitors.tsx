...
import { Grid2X2, ChevronRight, ExternalLink, CheckCircle2, Info } from "lucide-react";
import { useBrand } from "../context/BrandContext";
import {
  SectionKeyCompetitors,
  SectionCompetitivePosition,
  DEFAULT_DATA,
  WizardData,
...
function EmptyStateBanner() {
  return (
    <div className="mb-6 p-4 bg-[#00E5FF] border-[1.5px] border-black rounded-[4px] shadow-[3px_3px_0px_0px_#000] flex items-center gap-3">
      <Grid2X2 className="w-5 h-5 flex-shrink-0" />
      <div className="flex-1 min-w-0">
        <p className="text-[12px] font-bold">
          No competitor data yet. Add your key competitors and map your competitive position below.
        </p>
      </div>
      <Link
        to="/"
...
export function BrandKeyCompetitors() {
  ...
  const position = data.competitivePosition;
    ...
    <div className="flex flex-col max-w-3xl mx-auto pb-16">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 flex-shrink-0 flex items-center justify-center border-[2px] border-black rounded-[4px] shadow-[3px_3px_0px_0px_#000] font-['Archivo_Black'] text-[11px] bg-[#00E5FF]">
            B2–3
          </div>
          <div>
            ...
            <p className="text-sm font-bold text-gray-500 mt-0.5">
              Map the competitive landscape and define your strategic position
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3 flex-shrink-0">
          {namedCompetitors.length > 0 && (
            <div className="text-[11px] font-bold text-gray-600 bg-white border-[1.5px] border-black px-2.5 py-1 rounded-[4px] shadow-[2px_2px_0px_0px_#000]">
              {namedCompetitors.length} Competitor{namedCompetitors.length !== 1 ? "s" : ""}
            </div>
          )}
          {position && (
            <div
              className="text-[11px] font-bold border-[1.5px] border-black px-2.5 py-1 rounded-[4px] shadow-[2px_2px_0px_0px_#000]"
              style={{ backgroundColor: POSITION_COLORS[position] ?? "#FFDE00" }}
            >
              {position}
            </div>
          )}
      ...
      <div className="mb-4 flex items-start gap-2.5 px-4 py-3 bg-white border-[1.5px] border-black/20 rounded-[4px] text-[11px] font-bold text-gray-500">
        <Info className="w-4 h-4 flex-shrink-0 mt-0.5 text-[#00E5FF]" />
        <span>
          Add up to 5 competitors, then toggle the <span className="text-black">Perceptual Map</span> to visually
          position brands on custom axes. Your Competitive Position defines the strategic battle you&apos;re fighting.
        </span>
      </div>

      {/* B2 — Key Competitors */}
      <SectionKeyCompetitors data={data} onChange={handleChange} />

      {/* B3 — Competitive Position */}
      <div className="mt-5">
        <SectionCompetitivePosition data={data} onChange={handleChange} />
      </div>
